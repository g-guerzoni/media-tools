...
def test_download_out_of_registry_outcome_code_fails_only_that_entry(tmp_path, monkeypatch):
    """Residual of C3, applied to download's own item loop (the same exposure
    core.runner.run_items had: state.update()/reporter.item() were called outside any
    guard, so a status/reason/warning code outside the closed registry raised a KeyError
    that escaped _download_all entirely instead of failing only that entry)."""
    calls = {"n": 0}

    def fake_download_one(entry, **kwargs):
        calls["n"] += 1
        if calls["n"] == 1:
            return Outcome(status="done", outputs=[], bytes_out=0, reason="not_a_real_reason")
        output_dir = kwargs["output_dir"]
        target = output_dir / f"{entry.name}.mp4"
        target.write_bytes(b"x")
        return Outcome(status="done", outputs=[target], bytes_out=target.stat().st_size)
    ...
    data = json.loads((batch_dir / "run.json").read_text())
    items = {item["id"]: item for item in data["items"]}
    assert items[1]["status"] == "failed"
    assert items[1]["reason"] == "engine_error"
    # the second, well-behaved entry still completed
    assert items[2]["status"] == "done"
    assert (batch_dir / "item2.mp4").read_bytes() == b"x"
